...
class Discriminator1(BaseModule):
    def __init__(self,
                 in_channels: int = 4,
    ...
    def forward(self, voxel_features: torch.Tensor, coors: torch.Tensor) -> torch.Tensor:
        ...
        scores = []
        # 获取batch_size
        unique_batches = torch.unique(x.C[:,3])
        # batch_size = int(coors[-1, -1]) + 1 if self.sparseconv_backend != 'spconv' else int(coors[-1, 0]) + 1

        # 对于每个batch，我们计算各自的全局平均池化结果
        for b in unique_batches:
            if self.sparseconv_backend == 'torchsparse':
                mask = (x.C[:, -1] == b)
                batch_feature = x.F[mask]
                global_avg_pool = batch_feature.mean(dim=0)
            elif self.sparseconv_backend == 'spconv':
                mask = (x.C[:, 0] == b)
                batch_feature = x.features[mask]
                global_avg_pool = batch_feature.mean(dim=0)
