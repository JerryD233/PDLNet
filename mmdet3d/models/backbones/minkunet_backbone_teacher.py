...
class MinkUNetBackbone_teacher(BaseModule):
    ...
    def __init__(self,
        ...
        # # 初始化目标域采样器
        # self._init_target_sampler()
    
    def _init_target_sampler(self):
        """初始化目标域采样器（只在开始时执行一次）"""
        # 使用独立RNG（基于rank初始化）
        self.target_rng = np.random.default_rng(42 + self.rank)
        
        # 所有可用索引
        all_indices = np.arange(len(self.data_dicts['data_list']))
        
        # 分片策略：每卡获取不同的数据段
        per_rank = len(all_indices) // self.world_size
        self.target_indices = all_indices[self.rank * per_rank : (self.rank + 1) * per_rank]
        
        # 初始打乱
        self._shuffle_target_indices()
        
        # 当前指针位置
        self.target_ptr = 0
    
    def _shuffle_target_indices(self):
        """打乱当前卡的目标域索引（保持多卡差异）"""
        self.target_rng.shuffle(self.target_indices)
    
    def _get_target_samples(self, num_samples):
        """获取指定数量的目标域样本（自动循环）"""
        sampled_indices = []
        
        remaining = num_samples
        while remaining > 0:
            available = len(self.target_indices) - self.target_ptr
            take = min(remaining, available)
            
            # 获取当前可用的索引
            sampled_indices.extend(
                self.target_indices[self.target_ptr : self.target_ptr + take]
            )
            self.target_ptr += take
            remaining -= take
            
            # 如果到达末尾，重新打乱并从头开始
            if self.target_ptr >= len(self.target_indices):
                self._shuffle_target_indices()
                self.target_ptr = 0
    ...
    def forward(self, voxel_features: Tensor, coors: Tensor) -> Tensor:
        ...
        """
        if self.training and self.use_target and (self.adain or self.crossattention):
            # print(len(torch.unique(coors[:,-1])))
            rain_data = []
            
            # 源域的batch len（通过coors的unique数量获取）
            source_batch_len = len(torch.unique(coors[:, -1]))
            
            # 获取对应数量的目标域样本（自动循环）
            # target_indices = self._get_target_samples(source_batch_len)
            target_indices = np.random.choice(len(self.data_dicts['data_list']), size=source_batch_len, replace=False)
            # print(self.rank, ': ', target_indices, 'remain: ', len(self.target_indices) - self.target_ptr)
            # for i in range(len(torch.unique(coors[:,-1]))):
                # idx = idxs[i]
            ...
            # # T -> S
            # origin_rain_voxel_features = rain_voxel_features  # 用于add
            # rain_voxel_features = self.crossAttention(rain_voxel_features, origin_voxel_features, style_batch_indices, content_batch_indices)
            # rain_voxel_features = torch.add(origin_rain_voxel_features, rain_voxel_features)  # 用于add
        ...
        if self.sparseconv_backend == 'spconv':
            ...
            # return decoder_outs[-1].F, voxel_features, coors, rain_data, result
            if self.training and self.use_target:
                return decoder_outs[-1].F, voxel_features, coors, rain_data, result
            return decoder_outs[-1].F, None, None, None, None
