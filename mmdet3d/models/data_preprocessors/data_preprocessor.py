...
class Det3DDataPreprocessor(DetDataPreprocessor):
        ...
        if self.voxel_type == 'hard':
            ...
            for i, res in enumerate(points):
                ...
                if self.training and self.max_voxels is not None:
                    ...
                    # if len(inds) > self.max_voxels:
                    #     # 随机选择 inds，使其不超过 max_voxels
                    #     selected_indices = np.random.choice(np.arange(len(inds)), self.max_voxels, replace=False)
                    #     new_inds = inds[selected_indices]
                        
                    #     # 使用 np.argsort 来重新生成逆映射
                    #     old_to_new_map = {old_idx: new_idx for new_idx, old_idx in enumerate(selected_indices)}
                    #     new_point2voxel_map = np.array([old_to_new_map[idx] if idx in old_to_new_map else -1 for idx in point2voxel_map.cpu().numpy()])
                        
                    #     # 过滤掉无效索引（即那些不在 old_to_new_map 中的索引）
                    #     valid_mask = new_point2voxel_map >= 0
                    #     point2voxel_map = torch.from_numpy(new_point2voxel_map[valid_mask]).cuda()

                    #     inds = torch.from_numpy(new_inds).cuda()
                    # else:
    ...
    @torch.no_grad()
    def all_voxelize(self, points: List[Tensor],
                 data_samples: SampleList) -> Dict[str, Tensor]:
        """Apply voxelization to point cloud.
