...
class Base3DSegmentor(BaseModel, metaclass=ABCMeta):
    ...
    def postprocess_teacher_result(self, seg_logits_list: List[Tensor],
                           batch_data_samples: SampleList, 
                           confidence_threshold: float = 0.9) -> SampleList:
        """Convert results list to `Det3DDataSample`.
        ...
        Args:
            ...
            batch_data_samples (List[:obj:`Det3DDataSample`]): The det3d data
                samples. It usually includes information such as `metainfo` and
                `gt_pts_seg`.
            confidence_threshold (float): Confidence threshold for selecting pseudo labels.
        ...
        for i in range(len(seg_logits_list)):
            ...
            probs = F.softmax(seg_logits, dim=0)
            
            # 找到每个点的最大概率及其对应的预测类别
            max_probs, preds = torch.max(probs, dim=0)

            # 根据置信度阈值生成伪标签
            pseudo_labels = preds.clone()
            # 将低于置信度阈值的预测设置为无效（例如-1）
            pseudo_labels[max_probs < confidence_threshold] = 19

            # 存储结果回到batch_data_samples
            batch_data_samples[i].set_data({
                'pts_seg_logits':
                PointData(**{'pts_seg_logits': seg_logits}),
                'pred_pts_seg':
                PointData(**{'pts_semantic_mask': pseudo_labels})
            })
