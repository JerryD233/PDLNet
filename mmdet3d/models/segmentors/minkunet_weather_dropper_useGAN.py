...
class MinkUNetWeatherDropper_useGAN(EncoderDecoder3D):
    ...
    def __init__(self,
                 ...
                 n_actions=1,
                 **kwargs) -> None:
        super(MinkUNetWeatherDropper_useGAN, self).__init__(**kwargs)
    ...
    def get_drop_points(self, cache):
        ...
        for i in range(len(batch_inputs['points'])):
            ...
            action = action.to(enum.device) # 为解决indices should be either on cpu or on the same device as the indexed tensor (cpu)，保证action在cuda上
