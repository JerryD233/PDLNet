...
class MinkUNetWeatherDropper_direct_segonly(EncoderDecoder3D):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        super(MinkUNetWeatherDropper_direct_segonly, self).__init__(**kwargs)
    ...
    def adainCrossAttention(self, voxel_features: Tensor, coors: Tensor) -> Tensor:
        """Forward function.
        ...
        # 对晴朗天气和恶劣天气的特征使用AdaIN和CrossAttention
        ...
        return voxel_features, coors, rain_voxel_features, rain_coors
    ...
    def train_step(self, data: Union[dict, tuple, list],optim_wrapper: OptimWrapper) -> Dict[str, torch.Tensor]:
        ...
        for epoch in epochs:
            for input, target in data:
                ...
                scaler.update()
        """
        # print('cus_train_step')
        data = self.data_preprocessor(data, True)

        ########################## direct ###########################
        voxel_dict = data['inputs']['voxels'].copy()
        generated_voxel_features, _, _, _ = self.adainCrossAttention(voxel_dict['voxels'], voxel_dict['coors'])
        # 更新分割网络的数据
